...
export function AdminAgenda({ events }: AdminAgendaProps) {
  ...
  return (
    <section className="card" style={{ padding: 20 }} aria-labelledby="admin-agenda-title">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 16 }}>
        <div>
          <h2 id="admin-agenda-title" style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-primary)' }}>Agenda administrativa</h2>
          ...
          return (
            <button
              ...
              style={{
                ...
                border: 'none',
                borderRadius: 12,
                background: selected ? '#4f46e5' : isToday ? 'rgba(79, 70, 229, 0.1)' : 'transparent',
                color: selected ? '#fff' : isToday ? '#4f46e5' : 'var(--text-primary)',
                display: 'flex',
            ...
            >
              ...
              <span aria-hidden="true" style={{ width: 5, height: 5, borderRadius: '50%', background: hasEvents ? (selected ? '#fff' : '#4f46e5') : 'transparent' }} />
            </button>
          );
        })}
      </div>
      ...
      {selectedEvents.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {selectedEvents.map((event) => (
            <div key={event.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 12px', borderRadius: 12, background: '#f8fafc', borderLeft: '3px solid #4f46e5' }}>
