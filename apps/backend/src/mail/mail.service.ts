...
export class MailService {
  ...
  async sendInviteLink(
    ...
    const html = `
      ...
      <p>Voce foi convidado por <strong>${empresa}</strong> para realizar seu exame ocupacional.</p>
    ...
    await this.mailerService.sendMail({
      to,
      subject: `Convite - Exame Ocupacional - ${empresa}`,
      html,
    });
  }
  ...
  async sendAsoExpirationAlert(
    ...
    await this.mailerService.sendMail({
      to,
      subject: `ASOs perto do vencimento - ${empresa}`,
      html,
    });
  }
}
