    ...
    async login(email, password) {
        ...
        if (user.role === 'DOCTOR') {
            if (!user.doctorProfile?.verifiedAt) {
                throw new common_1.UnauthorizedException('Cadastro pendente de aprovação pela administração.');
            }
            profileId = user.doctorProfile.id;
        }
        else if (user.role === 'COMPANY_ADMIN')
            profileId = user.companyAdminProfile?.companyId ?? null;
        else if (user.role === 'OPERATOR')
            profileId = user.operatorProfile?.id ?? null;
        else if (user.role === 'CLINIC')
            profileId = user.clinicProfile?.id ?? null;
        else if (user.role === 'PATIENT')
            profileId = user.patientProfile?.id ?? null;
