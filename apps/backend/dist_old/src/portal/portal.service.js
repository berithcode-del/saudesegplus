...
let PortalService = class PortalService {
    ...
    companyGateway;
    storage;
    constructor(prisma, jwtService, queueService, companyGateway, storage) {
        this.prisma = prisma;
    ...
    async auth(token, cpf, birthDate) {
        ...
        if (!invite || invite.status === 'EXPIRADO') {
            throw new common_1.UnauthorizedException('Convite inválido ou expirado');
        }
        const normalizeCpf = (v) => v.replace(/\D/g, '');
        if (normalizeCpf(invite.expectedCpf ?? '') !== normalizeCpf(cpf)) {
            throw new common_1.UnauthorizedException('Dados não conferem ou link expirado.');
        }
        ...
        let examRequest = invite.examRequest ?? null;
        if (!patient) {
        ...
        if (!examRequest) {
            const resolvedClinic = await this.findBestClinicForInvite(invite.companyId, invite.clinicId ?? invite.company.clinicId ?? null);
            ...
            await this.prisma.examInvite.update({
                where: { id: invite.id },
                data: { status: client_1.InviteStatus.ABERTO, openedAt: new Date() },
            });
            ...
            this.companyGateway.emitInviteStatusChange(invite.companyId, {
                inviteId: invite.id,
                status: client_1.InviteStatus.ABERTO,
                examStatus: examRequest.status,
            });
        }
        if (!examRequest) {
            throw new common_1.NotFoundException('Solicitação de exames não encontrada');
        }
        const sessionToken = this.jwtService.sign({ sub: patient.id, processId: examRequest.id, role: 'PORTAL' }, { secret: (0, jwt_secret_1.getJwtSecret)(), expiresIn: (0, jwt_secret_1.getPortalJwtExpiresIn)() });
    ...
    birthDatesMatch(storedDate, submittedDate) {
        return this.birthDateCandidates(storedDate).has(submittedDate);
    }
    birthDateCandidates(date) {
    ...
    async getProcesso(patientId, processId) {
        ...
        const latestVideoSessionId = this.withEmbeddedJitsiConfig(request.teleconsultations[0]?.videoSessionId ?? null);
        ...
        const documentos = request.documents.map(d => ({
            tipo: d.tipo,
            enviado: true,
            fileUrl: d.fileUrl,
        }));
    ...
    calcularProximaAcao(status, extras) {
    ...
    async verificarDocumentosObrigatorios(patientId, processId) {
    ...
    calcularProgresso(status, hasAnamnese, documentos) {
    ...
    async confirmarDados(processId, patientId, phone, email) {
        const request = await this.prisma.examRequest.findUnique({
            where: { id: processId },
            include: { invite: true },
        });
    ...
    async getStatusDocumentos(patientId, processId) {
        const docs = await this.prisma.patientDocument.findMany({
            where: { patientId, requestId: processId },
        });
    ...
    async enviarDocumento(processId, patientId, tipo, fileUrl) {
        const request = await this.prisma.examRequest.findUnique({
            where: { id: processId },
            include: { invite: true },
        });
    ...
    async responderQuestionario(processId, patientId, data) {
        const request = await this.prisma.examRequest.findUnique({
            where: { id: processId },
        ...
        if (!data.declaracaoVeracidade) {
            throw new common_1.BadRequestException('Confirme a veracidade das informações para continuar');
        }
        if (data.tabagismo !== 'nao' && !data.tabagismoDetalhe?.trim()) {
            throw new common_1.BadRequestException('Informe o detalhe do tabagismo para continuar');
        }
        const anamneseData = {};
        if (data.queixas)
            anamneseData.queixas = data.queixas;
        if (data.medicamentosEmUso)
            anamneseData.medicamentos = data.medicamentosEmUso;
        if (data.observacoes)
            anamneseData.historicoOcupacional = data.observacoes;
        ...
        if (data.sono)
            habitosParts.push(`Sono: ${data.sono}`);
        if (habitosParts.length > 0)
            anamneseData.habitos = habitosParts.join('\n');
        const ocupacionalParts = [];
        if (data.observacoes)
            ocupacionalParts.push(data.observacoes);
        if (data.declaracaoVeracidade) {
    ...
    async getAso(processId, patientId) {
        const request = await this.prisma.examRequest.findUnique({
            where: { id: processId },
    ...
    async getAsoFile(processId, patientId) {
    ...
    async findBestClinicForInvite(companyId, preferredClinicId) {
