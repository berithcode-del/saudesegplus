...
function getStoredClinicId() {
  ...
  if (!clinicId && token) {
    try {
      ...
      clinicId = payload?.profileId || payload?.clinicId || '';
    } catch {
      clinicId = '';
    }
  }
...
export default function ConsultorioAsosPage() {
  const [clinicId, setClinicId] = useState('');
  const [asos, setAsos] = useState<ClinicAso[]>([]);
  const [loading, setLoading] = useState(true);
  const [viewer, setViewer] = useState<{ url: string; aso: ClinicAso } | null>(null);
    ...
    <>
      <div className="page-header">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '16px' }}>
          <div>
            <h2>ASOs emitidos pela clínica</h2>
            <p>Histórico de colaboradores com ASO ativo, ordenado por vencimento</p>
          </div>
      ...
      <div className="stats-grid" style={{ gridTemplateColumns: 'repeat(3, minmax(160px, 1fr))' }}>
        ...
        <div className="stat-card">
          <div className="stat-label">Até 7 dias</div>
          <div className="stat-value" style={{ color: '#dc2626' }}>{summary.critical}</div>
          <div className="stat-sub">renovação prioritária</div>
        </div>
        <div className="stat-card">
          <div className="stat-label">Até 30 dias</div>
      ...
      {viewer && (
        <div className="modal-overlay" onClick={() => { URL.revokeObjectURL(viewer.url); setViewer(null); }}>
          <div className="modal-content" style={{ width: 'min(1100px, 94vw)', height: '88vh', borderRadius: '16px', overflow: 'hidden' }} onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <div>
                <h3>ASO de {viewer.aso.collaborator.name}</h3>
                <p style={{ margin: 0, fontSize: '13px', color: '#6b7280' }}>Emitido em {formatDate(viewer.aso.issuedAt)} · Válido até {formatDate(viewer.aso.validUntil)}</p>
              </div>
              <button className="modal-close" onClick={() => { URL.revokeObjectURL(viewer.url); setViewer(null); }}>
                <XMarkIcon className="icon" />
              </button>
            </div>
            <iframe src={viewer.url} title="Visualização do ASO" style={{ width: '100%', height: 'calc(100% - 80px)', border: 'none' }} />
          </div>
        </div>
      )}
    </>
