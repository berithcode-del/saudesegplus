...
export default function ConsultorioLayout({
  children,
}: {
  ...
  useEffect(() => {
    apiFetch("/api/auth/me")
      .then((result: any) => {
        ...
        setIsSandbox(
          result?.clinicProfile?.environment === "SANDBOX" ||
            result?.operatorProfile?.clinic?.environment === "SANDBOX",
        );
        if (
          currentRole === "CLINIC" &&
          ![
            "/consultorio/selecionar-perfil",
            "/consultorio/configuracoes",
          ].includes(pathname)
        ) {
  ...
  const visibleNavItems = navItems.filter((item) => {
    if (role === "OPERATOR" && item.href === "/consultorio/financeiro")
      return false;
    if (
      role === "DOCTOR" &&
      [
        ...
        {activeActor && pathname !== "/consultorio/selecionar-perfil" && (
          ...
          >
            <span style={{ color: "#3730a3", fontSize: 13 }}>
              <strong>Profissional ativo:</strong> {activeActor.name} ·{" "}
              {activeActor.type === "DOCTOR" ? "Médico" : "Operador"}
            </span>
