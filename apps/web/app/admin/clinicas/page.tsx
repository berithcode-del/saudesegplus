...
export default function AdminClinicasPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...form,
        email: form.email.trim() + '@saudeseg.com',
  ...
  const handleSetMatriz = async (clinicId: string, setAsMatriz: boolean) => {
    const action = setAsMatriz ? 'definir como Matriz' : 'remover status de Matriz';
    if (!window.confirm(`Tem certeza que deseja ${action} esta clínica?\n\n${setAsMatriz ? 'A clínica se tornará a prioritária para receber pacientes quando a geolocalização não encontrar uma clínica mais próxima.' : 'A clínica deixará de ser a clínica prioritária (Matriz) para o estado.'}`)) {
      return;
    }
    setSettingMatriz(clinicId);
    try {
      await apiAdminSetClinicMatriz(clinicId, setAsMatriz);
      await fetchClinics();
    } catch (err) {
      alert(err instanceof Error ? err.message : 'Erro ao atualizar clínica.');
    } finally {
      ...
      <div className="card">
        {loading ? (
          ...
          <div style={{ color: '#dc2626', textAlign: 'center', padding: '40px' }}>
            <p style={{ marginBottom: '12px', fontWeight: 600 }}>Não foi possível carregar as clínicas.</p>
          ...
          <table className="queue-table">
            <thead>
              <tr><th>Nome</th><th>Cidade</th><th>Estado</th><th>CNPJ</th><th>Tipo</th><th>Empresas</th><th>Ações</th></tr>
            </thead>
            <tbody>
              {clinics.map(c => (
                <tr key={c.id}>
                  ...
                  <td>
                    {c.isMatriz ? (
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', padding: '2px 8px', background: '#fef3c7', color: '#92400e', borderRadius: '9999px', fontSize: '12px', fontWeight: 600 }}>
                        <BuildingStorefrontIcon style={{ width: 14, height: 14 }} /> Matriz
                      </span>
                    ) : (
                      <span style={{ color: '#6b7280', fontSize: '13px' }}>Filial</span>
                    )}
                  </td>
                    ...
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                      ...
                      {c.isMatriz ? (
                        <button
                          ...
                          onClick={() => handleSetMatriz(c.id, false)}
                          disabled={settingMatriz === c.id}
                        >
                          {settingMatriz === c.id ? <ArrowPathIcon className="icon" style={{ width: 14, height: 14, animation: 'spin 1s linear infinite' }} /> : 'Remover Matriz'}
                        </button>
                      ) : (
                        <button
                          className="btn btn-success"
                          style={{ padding: '4px 8px', fontSize: '12px' }}
                          onClick={() => handleSetMatriz(c.id, true)}
                          disabled={settingMatriz === c.id}
                        >
                          {settingMatriz === c.id ? <ArrowPathIcon className="icon" style={{ width: 14, height: 14, animation: 'spin 1s linear infinite' }} /> : 'Definir Matriz'}
                        </button>
                      )}
                    </div>
                  </td>
